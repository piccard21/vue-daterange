    ...
    >
      <div class="calendar-header">
        <div
          ...
          @click="changeMonth(1)"
        >
          <svgicon icon="arrow-left" width="7.4" height="12"></svgicon>
        </div>
        ...
        <div
          ...
          @click="changeMonth(-1)"
        >
          <svgicon icon="arrow-right" width="7.4" height="12"></svgicon>
        </div>
      </div>
      <div class="calendar-days-name">
        <div class="day" v-for="(day, i) in firstWeek" :key="i">
          <span>{{ day.name }}</span>
        </div>
      </div>
...
export default {
  ...
  watch: {
    currentPanel: {
      immediate: true,
      handler() {
        this.weekSelector = this.panel === "range" ? false : true;
        this.updateCalendar();
      }
    },
    preset(preset) {
